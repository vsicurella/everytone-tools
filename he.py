...
class HarmonicEntropy:
    ...
    def __init_members__(self):
        ...
        self.x_res = None
        self.axes_windows = None # tuple or tuple[]
        ...
        self.limit = None # cents limit of harmonic_limit
        self.harmonic_limit = None
    ...
    def __init__(self, basis_params=None, spread=15, alpha=7, x_res=1, he3=None, N=None, harmonic_limit=None, prime_limit=None, **kwArgs):
        ...
        if basis_params is None:
            c_limit = None if "complexity_limit" not in kwArgs else kwArgs["complexity_limit"]
            if harmonic_limit is None:
                harmonic_limit = 2 if he3 else 4
            if prime_limit is not None:
                basis_params = PrimeLimitBasisParams(prime_limit, harmonic_limit, c_limit, N, he3, False)
            # elif N is None and prime_limit is None:
            else:
                basis_params = IntegerBasisParams(100, harmonic_limit, c_limit, he3, False)

        args["basis_params"] = basis_params
        ...
        #     basis_params.prime_limit = self.kwArgs["p_limit"]
        ...
        for kw in args: 
            ...
            if kw == "basis_params":
                if self.basis is None:
                    self.basis = Basis(value)
                else:
                    self.basis.set_basis_params(value)
            elif kw == "x_res" and self.x_res != value:
            ...
            # if kw == "limit" and self.limit != value:
            #     self._vprint(3, f"Updated {kw}: {value}")
            #     updated = True
            #     self.limit = value
            #     self.updateX = True
            #     self.regenBasis = True
            # elif kw == "N" and self.N != value:
            #     self._vprint(3, f"Updated {kw}: {value}")
            #     updated = True
                # self.N = value
                # self.regenBasis = True
    ...
    def _prepare_x_axis(self):
        basis_params = self.basis.basis_query
        # self.harmonic_limit = int(np.round(np.exp2(self.limit / 1200)))
        # self.basis.set_basis_params(params)
        self.limit = ratio_to_cents(basis_params.harmonic_limit)
        # self.updateBasis = True

        self.x_axis = np.arange(0, int(np.ceil(self.limit+self.x_res)), step=self.x_res)
    ...
    def _weight_basis(self):
        ...
        if self.updateAlpha:
            self.basis_weights = np.reciprocal(self.basis.he_weights)
            self.basis_weight_alphas = np.reciprocal(self.basis.he_weights) ** self.a

        # self.updateWeights = False
        # self.updateDistribution = True

    def _get_mask_mesh(self):
        return np.meshgrid(np.arange(0, int(np.ceil(self.x_length * 1.5)), 1), np.arange(0, self.x_length, 1))
        ...
        if option == 'default':
            ...
            if self.he3:
                def linHe3Tx(periods):
                    indexes = np.rint((periods[:,1:] / periods[:,:-1] - 1) / self.harmonic_limit * (self.x_length - 1))
                    tx = np.rint((indexes[:,0] + (indexes[:,1] / 2)) / self.x_res).astype(np.uint32)
                    ty = np.rint(indexes[:,1] * self.tri_y_scalar / self.x_res).astype(np.uint32)
                    return (tx, ty)
                tx = linHe3Tx
            else:
                tx = lambda periods: np.rint(((periods[:,1] / periods[:,0]) - 1) / self.harmonic_limit * (self.x_length-1)).astype(np.uint32)
            pass
        elif option == 'polar':
            if self.he3:
                ...
                def polHe3Tx(periods):
                    ...
                    theta = norm[:,0] * np.pi * self.harmonic_limit
    ...
    def _transform_basis(self):
        if self.basis_transform_option is None:
            self.setTransformOption("default")

        self._vprint(1, 'Transforming basis...')
        if self.basis_transform_func is None:
            cents = self.basis.getCents()
            if self.he3:
                self.basis_triad_x = np.round((cents[:,0] + (cents[:,1] / 2)) / self.x_res).astype(np.int64)
                self.basis_triad_y = np.round(cents[:,1] * self.tri_y_scalar / self.x_res).astype(np.int64)
                self.basis_transform = (self.basis_triad_y, self.basis_triad_x)
            else: 
                self.basis_transform = np.rint(cents / self.x_res).astype(np.int64)
        else:
            self.basis_transform = self.basis_transform_func(self.basis.getValues())
    ...
    def _distribute_basis(self, alt_transform=None, alt_weights=None): # hacky temp solution
        ...
        transform = self.basis_transform if alt_transform is None else alt_transform
    ...
    def calculate(self):
        if self.updateX:
            self._prepare_x_axis()
        if self.basis.updateBasis():
            self.he3 = self.basis.he3

            if self.basis_distribution is None:
                newShape = self.x_length
                if self.he3:
                    newShape = (self.x_length, int(np.ceil(self.x_length * 1.5)))
                self.basis_distribution = np.zeros(newShape)
                self.basis_distribution_alpha = np.zeros(newShape)
            else:
    ...
    def getEntropyPlotData(self, min_cents=None, max_cents=None):
        ...
        if min_cents is not None:
            start = max(0, int(np.round(min_cents / self.x_res)))

        end = self.x_length
        if max_cents is not None:
            end = min(self.x_length, int(np.round(max_cents / self.x_res)))
    ...
    def makePlot(self, save=True, show=True, **hePlotArgs):
        ...
        if ticksKw in self.kwArgs and self.kwArgs[ticksKw]:
            ...
            tickLabels = [ f"{(t*self.x_res):.2f}" for t in ticks ]
        ...
        annotation = f"s={self.s}, a={self.a}"
        if self.x_res != 1:
            annotation += f", res={self.x_res}c"
...
if __name__ == "__main__": 
    ...
    parser.add_argument('-r', '--x_res', type=float, help="Resolution of x-axis in cents")
    # parser.add_argument('-l', '--limit', type=float, help="Last cents value to calculate")

    parser.add_argument('-n', '--N', type=int, help="Integer limit of basis set ratio denominators, will be multipled by harmonic-limit. Default is 100")
    parser.add_argument('-l', '--harmonic-limit', type=int, help="Harmonic to extend basis by (two octaves, or one for 3HE, by default)")
    parser.add_argument('-p', '--prime-limit', type=str, help='Filter basis set by this prime limit')
    parser.add_argument('-c-lim', '--complexity-limit', type=int, help='Filter basis set by max product of integers in ratios')
